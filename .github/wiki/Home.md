# Agentic Plugin Wiki

Welcome to the Agentic Plugin documentation wiki. For assistance please email support@agentic-plugin.com

## 📚 Documentation

### Getting Started
- [Quick Start Guide](Quickstart)
- [Roadmap](Roadmap) - Project timeline and milestones

### Architecture & Design
- [Architecture](Architecture) - Technical specifications and system design
- [Use Cases](Use-Cases) - Real-world scenarios and examples
- [Discussion Points](Discussion-Points) - Open questions and community input

### Development
- [Contributing Guidelines](Contributing)
- [Code of Conduct](Code-of-Conduct)
- [Security Policy](Security)
- [Agent Licensing for Developers](Agent-Licensing-for-Developers) - Build and monetize agents

### Features & Systems
- [Async Jobs System](Async-Jobs)
- [System Requirements Checker](System-Requirements-Checker)
- [WordPress.org Submission Checklist](WordPress-Org-Checklist)

### Release Information
- [Release Notes](Release-Notes)

### Technical Reports
- [WPCS Auto-Fix Report](WPCS-Auto-Fix-Report)
- [Exec Removal Summary](Exec-Removal-Summary)

## 🔗 Quick Links

- [GitHub Repository](https://github.com/renduples/agentic-plugin)
- [WordPress.org Plugin Page](https://wordpress.org/plugins/agentic-plugin/) (pending approval)
- [Official Website](https://agentic-plugin.com)

## 📝 Notes for Contributors

This wiki contains extended documentation that supplements the main README. For WordPress.org submission requirements, only the following files are maintained in the repository root:
- README.md (GitHub)
- readme.txt (WordPress.org format)
- LICENSE

All other documentation has been moved to this wiki for better organization.
